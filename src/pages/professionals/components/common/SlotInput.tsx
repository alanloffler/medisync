...
export const SlotInput = forwardRef<HTMLInputElement, { handleSlotInput: (slot: string) => void }>(({ handleSlotInput }, ref) => {
  const isAllowed = (values: { formattedValue: string; floatValue: number | undefined }) => {
    const { formattedValue, floatValue } = values;
    if (floatValue === undefined) return true;
  ...
  return (
    <div ref={ref}>
      <PatternFormat 
        ...
        mask='-' 
        isAllowed={isAllowed} 
        className='h-9 text-muted-foreground focus-visible:text-black'
       />
    </div>
  );
});
