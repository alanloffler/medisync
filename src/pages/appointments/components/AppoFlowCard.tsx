...
export function AppoFlowCard() {
  ...
  return (
    <Card className='flex items-center space-x-8 bg-amber-200 px-6 py-4 text-amber-600'>
      <section className='my-auto'>
        <ChartLine size={20} strokeWidth={2} className='stroke-amber-600' />
      </section>
      {!isError && (isLoading || flowValue === undefined) ? (
        <LoadingText text={t('loading.default')} suffix='...' className='!text-xsm text-amber-600' />
      ) : (
        flowValue?.count && (
          <NumberFlowGroup>
            <div className='flex flex-col font-semibold'>
              {flowValue.count && (
                <div className='flex items-center space-x-2'>
                  <NumberFlow className='fill-amber-600 text-2xl text-amber-600' format={{ style: 'decimal' }} value={flowValue.count.value} />
              ...
              <div className='flex items-center space-x-2'>
                <NumberFlow
                  className={cn(
                    '!text-xsm',
                    flowValue.diff ? (flowValue.diff < 0 ? 'fill-rose-500 text-rose-500' : 'fill-emerald-500 text-emerald-500') : 'text-amber-600',
                  )}
